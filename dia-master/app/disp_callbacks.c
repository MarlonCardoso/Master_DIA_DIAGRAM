            ...
            switch (kevent->keyval) {
              ...
              case GDK_KEY_A:
                if (kevent->state & GDK_CONTROL_MASK) {
                  ...
                  focus->text->has_selection = TRUE;
                  focus->text->sel_start_row = 0;
                  focus->text->sel_start_pos = 0;
                  focus->text->sel_end_row = dia_text_get_n_lines(focus->text) - 1;
                  focus->text->sel_end_pos = dia_text_get_line_strlen(focus->text, focus->text->sel_end_row);
                  diagram_flush(ddisp->diagram);
                } else {
                  handle_key_event (ddisp, focus, kevent->state, kevent->keyval, kevent->string, kevent->length);
                  diagram_flush(ddisp->diagram);
                }
                break;
              ...
              case GDK_KEY_C:
                if (kevent->state & GDK_CONTROL_MASK) {
                  ...
                  char *text_str = dia_text_get_selection_copy(focus->text);
                  if (text_str) {
                    GtkClipboard *clipboard = gtk_clipboard_get(GDK_SELECTION_CLIPBOARD);
                    gtk_clipboard_set_text(clipboard, text_str, -1);
                    g_free(text_str);
                  }
                } else {
                  handle_key_event (ddisp, focus, kevent->state, kevent->keyval, kevent->string, kevent->length);
              ...
              case GDK_KEY_KP_Left:
                if (kevent->state & GDK_SHIFT_MASK) {
                  /* Shift + Seta Esquerda */
                  if (!focus->text->has_selection) {
                    focus->text->has_selection = TRUE;
                    focus->text->sel_start_pos = focus->text->cursor_pos;
                    focus->text->sel_start_row = focus->text->cursor_row;
                  }
                  handle_key_event (ddisp, focus, kevent->state, kevent->keyval, kevent->string, kevent->length);
                  focus->text->sel_end_pos = focus->text->cursor_pos;
                  focus->text->sel_end_row = focus->text->cursor_row;
                } else {
                  focus->text->has_selection = FALSE;
                  handle_key_event (ddisp, focus, kevent->state, kevent->keyval, kevent->string, kevent->length);
                }
                diagram_flush(ddisp->diagram);
                break;
              ...
              case GDK_KEY_Tab:
                focus = textedit_move_focus (ddisp, focus, (state & GDK_SHIFT_MASK) == 0);
                obj = focus_get_object (focus);
                break;
