...
struct _DiaText {
  ...
  /* NOVOS CAMPOS PARA SELEÇÃO PARCIAL: */
  int sel_start_pos;
  int sel_start_row;
  int sel_end_pos;
  int sel_end_row;
              ...
              DiaAlignment  align)
{
  ...
  /* Inicialização dos pontos de seleção */
...
dia_text_draw (DiaText *self, DiaRenderer *renderer)
{
  ...
  if (DIA_IS_INTERACTIVE_RENDERER (renderer) && (self->focus.has_focus)) {
    ...
    /* DESENHA O DESTAQUE DE SELEÇÃO */
    if (self->has_selection) {
      double w_start, w_end, sel_x1, sel_x2;
      Point rect_p1, rect_p2;
      DiaColour blue_colour = { 0.2f, 0.4f, 0.8f, 1.0f };

      w_start = dia_renderer_get_text_width (renderer,
                                              dia_text_get_line (self, self->sel_start_row),
                                              self->sel_start_pos);
      w_end = dia_renderer_get_text_width (renderer,
                                            dia_text_get_line (self, self->sel_end_row),
                                            self->sel_end_pos);

      sel_x1 = self->position.x + MIN(w_start, w_end);
      sel_x2 = self->position.x + MAX(w_start, w_end);

      if (self->alignment == DIA_ALIGN_CENTRE) {
        sel_x1 -= str_width_whole / 2.0;
        sel_x2 -= str_width_whole / 2.0;
      } else if (self->alignment == DIA_ALIGN_RIGHT) {
        sel_x1 -= str_width_whole;
        sel_x2 -= str_width_whole;
      }

      rect_p1.x = sel_x1;
      rect_p1.y = curs_y;
      rect_p2.x = sel_x2;
      rect_p2.y = curs_y + height;

      dia_renderer_draw_rect (renderer, &rect_p1, &rect_p2, &blue_colour, &blue_colour);
    }

    /* DESENHA A LINHA PRETA DO CURSOR */
    p1.x = curs_x;
    p1.y = curs_y;
    p2.x = curs_x;
    p2.y = curs_y + height;
...
/**
 * dia_text_get_selection_copy:
 * Extract only the selected portion of text, or full text if no selection.
 */
char *
dia_text_get_selection_copy (DiaText *self)
{
  g_return_val_if_fail (DIA_IS_TEXT (self), NULL);

  if (!self->has_selection) {
    return dia_text_get_string_copy (self);
  }

  /* Caso a seleção esteja restrita a uma única linha */
  if (self->sel_start_row == self->sel_end_row) {
    const char *line = dia_text_get_line (self, self->sel_start_row);
    int start_p = MIN(self->sel_start_pos, self->sel_end_pos);
    int end_p = MAX(self->sel_start_pos, self->sel_end_pos);

    char *utf8_start = g_utf8_offset_to_pointer (line, start_p);
    char *utf8_end = g_utf8_offset_to_pointer (line, end_p);

    return g_strndup (utf8_start, utf8_end - utf8_start);
  }

  /* Fallback para cópia completa em múltiplas linhas */
  return dia_text_get_string_copy (self);
}